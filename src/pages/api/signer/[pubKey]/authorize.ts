...
export default async function handle(
  ...
  const appMnemonic = process.env.APP_MNEMONIC || process.env.APP_MNENOMIC;
  if (!appFid || !/^\d+$/.test(appFid) || !appMnemonic) {
    throw new Error('Farcaster app identity is not configured');
  }
