...
import { NeynarNotifications } from '@components/notifications/neynar-notifications';
...
export default function NotificationsPage(): JSX.Element {
  ...
  const {
    ...
    (pageIndex, prevPage) => {
      if (!user || user.neynarSignerUuid) return null;
  ...
  return (
    <MainContainer>
      <SEO title={`Notifications / SealedCast`} />
      <MainHeader
        useMobileSidebar
      ...
      {user?.neynarSignerUuid ? (
        <NeynarNotifications fid={user.id} />
      ) : !user?.keyPair ? (
