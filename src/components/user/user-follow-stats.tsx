...
type UserFollowStatsProps = Pick<
  UserFull,
  'following' | 'followers' | 'followingCount' | 'followerCount'
>;
...
export function UserFollowStats({
  ...
  const totalFollowing = followingCount ?? following.length;
  const totalFollowers = followerCount ?? followers.length;
