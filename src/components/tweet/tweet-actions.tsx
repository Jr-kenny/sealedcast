...
export function TweetActions({
  ...
  const handleRemove = async (): Promise<void> => {
    if (currentUser?.neynarSignerUuid && isOwner) {
      try {
        const response = await fetch('/api/neynar/delete-cast', {
          method: 'DELETE',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            fid: currentUser.id,
            signerUuid: currentUser.neynarSignerUuid,
            targetHash: tweetId
          })
        });
        const body = (await response.json()) as { error?: string };
        if (!response.ok) {
          throw new Error(body.error || 'Failed to delete cast');
        }
        removeCloseModal();
        toast.success('Your Cast was deleted');
