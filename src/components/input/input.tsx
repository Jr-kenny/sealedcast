...
export function Input({
  ...
  const sendTweet = async (): Promise<void> => {
    ...
    let sealedCastId: string | null = null;
    if (sealed) {
      if (selectedImages.length > 0) {
        toast.error(
          'Encrypted media is not available yet. Remove media before sealing this cast.'
        );
        setLoading(false);
        return;
      }
      if (!user || (!user.keyPair && !user.neynarSignerUuid)) {
        toast.error('Sign in before creating a Sealed Cast');
        setLoading(false);
        return;
      }
      try {
        ...
        toast.error(
          reason instanceof Error ? reason.message : 'Could not seal cast'
        );
        setLoading(false);
        return;
      }
    ...
    const usersMap = user?.neynarSignerUuid
      ? {}
      : (
          await fetchJSON<BaseResponse<{ [key: string]: number }>>(
      ...
      try {
        const response = await fetch('/api/neynar/cast', {
          ...
          body: JSON.stringify({
            ...
            parent:
              isReplying && parent
                ? parent.id.startsWith('0x')
                  ? parent.id
                  : `0x${parent.id}`
                : undefined,
            parentAuthorFid:
              isReplying && parent ? parseInt(parent.userId) : undefined,
            idem: crypto.randomUUID()
          })
        });
        const body = (await response.json()) as {
          result?: { cast: { hash: string } };
          error?: string;
        };
        if (!response.ok || !body.result?.cast.hash) {
          throw new Error(body.error || 'Could not publish the cast');
        }
        await sleep(500);
        if (!modal && !replyModal) discardTweet();
        setLoading(false);
        if (closeModal) closeModal();
        const tweetId = body.result.cast.hash.replace(/^0x/, '');
        toast.success(
          () => (
        ...
        setLoading(false);
        toast.error(
          reason instanceof Error ? reason.message : 'Could not publish cast'
        );
      }
      return;
    }
    ...
    const castMessage = await createCastMessage({
      ...
      fid: parseInt(userId),
      embeds: castEmbeds,
      mentions: sealed ? [] : mentions,
      mentionsPositions: sealed ? [] : mentionsPositions,
